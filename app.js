...
app
	.use(mongo({host:process.env["MONGODB_HOST"]||'localhost',port:process.env["MONGODB_PORT"]||27017,user:process.env["MONGODB_USER"]||'',pass:process.env["MONGODB_PASS"]||' ',db:process.env["MONGODB_DB"]||'test'}))
	.use(logger())
	.use(router.routes())
